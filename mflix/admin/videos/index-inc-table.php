    ...
    <div class="card-header border-0 pt-10">
      <div class="row w-100">
        ...
        <div class="col-lg-8 col-md-7 text-md-end">
          <a href="/mflix/admin/videos/manage" class="btn btn-mflix fw-bold" onclick="KTApp.showPageLoading()">
            <i class="bi bi-plus fs-2 text-white"></i>Add Video
          </a>
        </div>
      </div>
    </div>
  ...
  $(document).ready(function () {
    ...
    var MBGDATATABLE = $('#EdITH-TABLE').DataTable({
      ...
      columns: [
        ...
        { data: 4, searchable: false, visible: true },
        { data: 5, searchable: false, visible: true }
      ],
      "columnDefs": [
        {
          "targets": 0,
        ...
        },
        {
          ...
          render: function (data, type, row) {
            return `<span class="fw-bold text-gray-800">${row[1]}</span>`;
          }
        },
        {
          "targets": 2,
          render: function (data, type, row) {
            return `<span class="badge bg-light-danger text-mflix">${row[2]}</span>`;
          }
        },
        {
          "targets": 3,
          render: function (data, type, row) {
            return `<span class="text-gray-600"><i class="ki-outline ki-timer fs-7 me-1"></i>${row[3]}</span>`;
          }
        },
        {
          "targets": 4,
          "orderable": false,
          render: function (data, type, row) {
            let statusClass = row[4] === 'published' ? 'badge-light-success' : 'badge-light-warning';
            return `<span class="badge ${statusClass}">${row[4]}</span>`;
          }
        },
        {
          "targets": 5,
              ...
              <div>
                ...
                <div class="menu menu-sub menu-sub-dropdown menu-column menu-rounded menu-gray-800 menu-state-bg-light-primary fw-semibold w-auto min-w-200 mw-300px py-3" data-kt-menu="true">
                  <div class="menu-item px-3">
                    <a href="/mflix/video/${row[5]}" class="menu-link px-3" target="_blank">
                      <i class="bi bi-eye me-2"></i>View
                    </a>
                  </div>
                  <div class="menu-item px-3">
                    <a href="/mflix/admin/videos/manage/${row[0]}" class="menu-link px-3" onclick="KTApp.showPageLoading()">
                      <i class="bi bi-pencil-square me-2"></i>Edit
                    </a>
                  </div>
                  <div class="menu-item px-3">
                    <a href="javascript:void(0)" id="${row[0]}" class="menu-link px-3 menu-delete" table="mflix_videos">
